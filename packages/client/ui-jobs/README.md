...
Every `running` row carries a stop control named by its label. The click issues one fire-and-return kill under the issuing session's visibility — owned jobs authorize through their exact owner agent, unowned jobs are open to every caller — and never flips the row locally: the `stopping` transition arrives through the ordinary change push, so a still-running row keeps its control until the registry confirms. One request may be in flight at a time; while it is, every stop control disables. A rejected request displaces the row's status text with a failure word (raw message in the tooltip) until the next attempt or push. The host answers a requested stop by injecting a model-facing notice into the live owner, because the kill marks the record reported and would otherwise suppress its completion notice; it never wakes an idle driver.
...
None, as this package renders host-computed registry state for a human and touches no prompt, message, schema, stream, or tool result. The model's own view of the same jobs stays with [`dsh-tool-jobs`](../../jobs/tool-jobs/README.md); the human-kill notice the host injects on this package's behalf is host behavior, not client.
...
- **A killed job's output is unread here** — the stop control requests cancellation but offers no way to read what the job produced first; collecting a killed job's remaining output stays the owner agent's `job_output` business.
