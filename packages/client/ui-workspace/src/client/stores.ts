...
type WorkspaceViewState = {
  ...
  /** Fold state of the trailing Archived section (grouped tree and flat list share it). */
...
export function createWorkspaceViewStore(): EngineStoreHandle<WorkspaceViewState, WorkspaceViewActions> {
  return defineStore({
    ...
    // v6: added archivedExpanded. Persistence is whole-value JSON (a stored
    // value replaces the entire state), so a schema change bumps the key
    // instead of layering per-field migrations onto every reader.
