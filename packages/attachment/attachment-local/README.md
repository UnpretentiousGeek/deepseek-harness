...
Document admission rides the same service. Accepted formats are text/plain, text/markdown, text/csv, application/json, application/pdf, DOCX, and XLSX. Defaults accept at most 10 documents and 30MiB of encoded source per message, 10MiB per file, and 200,000 extracted characters per document — longer extractions are admitted and truncated with a visible `[Document truncated.]` marker instead of refused. Text-like formats are UTF-8 decoded with a binary guard (NUL bytes or dominant replacement glyphs refuse the file); PDFs go through the bundled pdf.js build, DOCX through mammoth's raw-text reader, and XLSX workbooks serialize as per-sheet TSV blocks. An empty extraction (empty file, scanned page) fails loudly rather than attaching nothing.
