...
export interface ApiRemoteAgentOptions {
  ...
  /**
   * Receive every handle a cold resume creates. Without it the resumed agent
   * is live but its disposal capability is lost — no caller can dispose it,
   * and owners of the live identity cannot tell the orphan from another
   * surface's owned agent.
   * @param sessionId - identity the handle was created for.
   * @param handle - the newly created handle.
   */
  readonly retain?: (sessionId: SessionId, handle: AgentHandle) => void
}
    ...
    if (resume === undefined) {
      resume = (async () => {
        try {
          ...
          options.retain?.(sessionId, handle)
          return handle.agent
        } finally {
