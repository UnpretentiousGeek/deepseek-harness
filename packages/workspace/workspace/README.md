...
- `ctx.workspaceRegistry.archiveSession(id)` / `unarchiveSession(id)` / `archivedSessionIds` — the registry-global archive set, layered over workspace accounting: an archived session disappears from grouping surfaces but keeps its session log and its `sessionIds` slot, so unarchiving restores its position. Archiving accepts any live or persisted session (accounted or Ungrouped), resolves without writing for an already archived id, and rejects an unknown id. Unarchiving removes one id while preserving the remaining archive order, resolves without writing for a non-archived id, and validates no existence (membership alone authorizes removal). State written before the field existed parses with an empty set. `ctx.workspaceRegistry.forgetSession(id)` purges one permanently deleted session from every registry fact — its accounting slot in each owning workspace record and its archive-set entry — idempotently for an id no record references.
