...
| `delete(id, signal?): Promise<boolean>` | Permanently remove one session's stored log and every backend artifact for its id. A live session refuses — dispose the owning Agent first; a draining disposal is awaited before the medium write. Unknown ids resolve to `false` without writing. After resolution the id is absent from `list` and rejected by `load`, and cannot be resurrected. |
