...
Archived sessions had no viewing or unarchive surface at introduction (this iteration's scope, recorded then as a README Known Limitation); data and accounting slots stay intact, so restore remained one UI surface plus one inverse RPC. That surface now exists — `workspace.unarchiveSession` and the sidebar's Archived section — and keeps every mechanism described here unchanged (see [the session-unarchive Agent Note](2026-08-21-workspace-session-unarchive.md)). The `workspace.list` response shape change is a pre-release direct edit (no compatibility layer). The workspace-management e2e pins the full chain (archive → row disappears → still hidden after reload, log still present); domain tests pin idempotence, unknown-id rejection, restart recovery, and the pre-field media default upgrade.
